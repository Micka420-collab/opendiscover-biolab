 ...
 * with the AURORA index, so a fuller planet visibly glows more. Rotation and ribbons
 * freeze to a static frame under reduced motion.
 */
...
/** A Fibonacci sphere of unit vectors — evenly spread points, deterministic. */
...
export function AuroraEarth({ litIds, index, reducedMotion, pulseKey, className }: Props) {
  ...
  const dots = useRef<Dot[]>(fibSphere(340));
  ...
  useEffect(() => {
    ...
    const draw = (t: number) => {
      ...
      // atmosphere halo, brighter with the index
      ...
      // aurora ribbons near the northern limb, scaling with the index
      if (!reduced && idx > 0) {
        const bands = 3;
      ...
      // ignition flash on a fresh lock
