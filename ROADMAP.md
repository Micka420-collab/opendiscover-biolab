...
- [x] **Lab workbench UI** — `/lab` catalog (domain-filterable) and `/lab/[engine]`
      interactive playground: a form generated by walking each engine's Zod schema
      (`src/lib/lab/params-form.ts`, unwraps `.refine`/`.default`/`.optional` down to
      typed number/enum/boolean controls, JSON fallback for nested params), wired to
      `/api/lab/run`, rendering metrics, an auto-built Vega-Lite chart from `series`
      (or the engine's own `vizSpec`), and the reproducibility hash. 5 tests on the
      schema-walker against real engine schemas. `/lab/campaigns` live viewer is
      split out below — still open.
- [ ] **Lab campaigns live viewer** — `/lab/campaigns` UI for autonomous campaign runs
